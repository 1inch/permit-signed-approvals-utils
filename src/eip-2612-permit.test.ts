...
describe('Eip2612PermitUtils', () => {
    ...
    describe('enabledCheckSalt', () => {

        beforeEach(() => {
            eip2612PermitUtils = new Eip2612PermitUtils(connector, { enabledCheckSalt: true });
        });

        it('Build permit signature without salt', async () => {
            const signature = await eip2612PermitUtils.buildPermitSignature(
                permitParams,
                chainId,
                tokenName,
        ...
        it('Build permit signature with salt', async () => {
